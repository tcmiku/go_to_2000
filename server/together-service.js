...
export function createTogetherService({now=Date.now}={}) {
  const rooms=new Map();
  function clean(){for(const [id,room] of rooms){for(const [token,member] of room.members)if(now()-member.seen>45000)room.members.delete(token);if(now()-room.touched>3600000)rooms.delete(id);}}
  ...
  return function handle(action,input={},token='') {
    clean();
    ...
    if(action==='create'){
      ...
      room={id,members:new Map(),track:null,playing:false,position:0,updatedAt:now(),touched:now(),revision:0};rooms.set(id,room);
    ...
    if(action==='create'||action==='join'){
      ...
      const token=randomBytes(24).toString('hex'),id=randomBytes(6).toString('hex');
      room.members.set(token,{id,name:text(input.name,20)||'听友',seen:now()});room.touched=now();
      return {...snapshot(room),token,memberId:id};
    }
    const member=room.members.get(token);if(!member)throw fail(401,'连接已过期，请重新加入房间');
    member.seen=now();room.touched=now();
    if(action==='leave'){room.members.delete(token);return {ok:true};}
    ...
    room.updatedAt=now();room.revision++;return snapshot(room);
  };
}
