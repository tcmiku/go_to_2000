...
import { createMusicWallService } from './music-wall-service.js';
...
['music-wall.html','music-wall.css','music-wall.js','music-wall-data.js','music-wall-canvas.js'].forEach(file=>publicFiles.add(file));
...
export function validateStore(data) {
  ...
  return {musicSources,navigation:data.navigation,content:data.content,settings:{tagline:data.settings.tagline,announcement:data.settings.announcement,ad}};
}
export async function createApp({dataDir = path.join(root,'data'), secureCookie = false, adminEnabled = true, adminPath = '/admin'} = {}) {
  ...
  let store;
  try {
    const stored=await readJSON('store.json'),valid=validateStore(stored);
    store={...valid,revision:stored.revision,updatedAt:stored.updatedAt};
    if(JSON.stringify(store)!==JSON.stringify(stored))await atomic('store.json',store);
  } catch(error) {
  ...
  function publicData(){const data=structuredClone(store);delete data.musicSources;data.navigation.categories.forEach(c=>{c.sites=c.sites.filter(s=>!s.hidden);(c.children||[]).forEach(x=>x.sites=x.sites.filter(s=>!s.hidden));});const ids=new Set(flattenCategories(data.navigation.categories).flatMap(c=>c.sites.map(s=>s.id)));data.content.hot.siteIds=data.content.hot.siteIds.filter(id=>ids.has(id));data.content.featured.items=data.content.featured.items.filter(s=>!s.hidden);data.content.friends=data.content.friends.filter(s=>!s.hidden);return data;}
  ...
  const musicWallService=createMusicWallService();
  ...
  return http.createServer(async(req,res)=>{
    ...
    try{
      ...
      if(route==='/api/listening/playlist') {
        if(req.method!=='GET')throw httpError(405,'请求方式不支持');
        return json(res,200,await musicWallService(url.searchParams.get('input')));
      }
      ...
      if(adminRoute||route==='/admin.html'){res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https: http:; connect-src 'self' https://v1.hitokoto.cn; base-uri 'self'; form-action 'self'; frame-ancestors 'self'");}
      ...
      if(route.startsWith('/api/admin')){
        ...
        if(route==='/api/admin/data'&&req.method==='PUT'){
          ...
          const saved=await locked(async()=>{
            ...
            let valid;try{valid=validateStore({...input,musicSources:input.musicSources===undefined?store.musicSources:input.musicSources});}catch(error){throw httpError(400,error.message);}
      ...
      const filename=route==='/'?'index.html':route==='/newsstand'?'newsstand.html':route==='/cassette-room'?'cassette-room.html':route==='/listening-room'?'listening-room.html':route==='/music-wall'?'music-wall.html':route==='/together'?'together.html':adminRoute?'admin.html':route.slice(1);
