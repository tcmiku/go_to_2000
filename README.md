...
同步服务位于 `server/together-service.js`，每个 Node 进程保存最多 300 个房间、每房间最多 20 人。浏览器每秒拉取服务端权威状态，以请求往返时间补偿和本地单调时钟推算位置；超过 1 秒的偏差直接校正，较小偏差用轻微变速追赶。操作采用版本校验，避免慢速音源解析覆盖其他人的新选曲。成员令牌不出现在分享链接或成员列表中。
