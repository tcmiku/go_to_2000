...
入口为 `/music-wall.html`（也支持 `/music-wall`），首页菜单提供链接。运行 `npm start` 后访问 `http://localhost:3000/music-wall.html`。

- 专辑封面无缝铺满画布，按视口复用真实专辑形成可无限平移的墙；仅创建视口附近的封面节点。鼠标拖动平移、滚轮缩放、左下角复位；支持手机单指平移和双指缩放。鼠标附近的封面按照距离连续放大，其他专辑加高斯模糊，移开后恢复；减少动态效果模式关闭放大。
- 封面中央的玻璃播放/暂停按钮直接控制音乐，不展开播放器；加载时显示可取消的旋转状态，正在播放的专辑保留控制按钮。点击封面其他位置展开播放器。触屏上直接显示封面按钮。
- 顶部「歌单」和播放器队列按钮打开曲目列表，可查看本地/导入歌单、新建歌单、把当前歌曲加入选定歌单并去重。点击「顺序播放」从第一首开始，点击其他曲目从该首继续；最后一首结束后停止。正在播放的队列独立于浏览中的专辑墙与歌单，列表编号遵循原始歌曲顺序，不受封面归组影响。可切换随机播放或单曲循环；自建歌单与曲目顺序保存在本浏览器。
- 点击封面展开液态玻璃播放器，桌面左侧为专辑封面、右侧为逐句歌词及进度条；手机采用纵向布局。支持专辑选曲、播放暂停、上一首/下一首、随机、单曲循环、音量、静音、进度拖动及点击歌词跳转。收起播放器后音乐继续播放。
- 与黑胶聆听室复用 `/api/radio` 本地音乐、管理员音源目录、LX 隔离运行环境、音源检测和歌词服务。初始展示本地唱片、黑胶收藏及真实热歌榜；本地封面按歌名与歌手精确匹配元数据。
- 通过网易云歌单 ID、公开歌单链接或分享短链接导入歌单，保留歌曲顺序并补齐分批返回的曲目；失败不保存半份歌单。每个歌单最多 1000 首、本浏览器最多 20 个歌单，重复导入更新同一歌单。搜索选中的歌曲也保存在本浏览器。
- 页面只保留操作名称、专辑/歌曲信息、歌词及必要的状态反馈，没有介绍或操作说明段落。

新增实现位于 `public/music-wall.html/css/js`、`public/music-wall-canvas.js`、`public/music-wall-data.js` 和 `server/music-wall-service.js`。单元与接口验证：`node --test tests/music-wall.test.js tests/listening.test.js tests/music-source-selection.test.js tests/server.test.js`。

2026-09-20 浏览器验证：本地播放、网络音源 QDY 实际播放、真实歌词与进度跳转、暂停/切歌/收起后继续播放、完整导入 200 首公开歌单及刷新保留、画布拖动不误触播放、滚轮缩放、触屏双指缩放、390px 手机布局。上述相关测试 54 项通过；全项目原有弹球测试 `three drains end the game and require a new game` 仍失败。
