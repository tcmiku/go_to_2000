...
function draw(){
  ...
  if(section==='sources')drawSources();else if(section==='sites')drawSites();else if(section==='submissions')drawSubmissions();else if(section==='categories')drawCategories();else if(section==='hot')drawHot();else if(section==='featured'||section==='friends')drawLinks();else if(section==='settings')drawSettings();else drawBackup();
}
function siteRows(){
...
function add(){if(section==='sources')openSource();else if(section==='sites')openSite();else if(section==='categories')openCategory();else if(section==='hot')openHot();else if(section==='featured'||section==='friends')openLink();}
...
$('#import-file').onchange=async event=>{
  ...
  try{if(file.size>8*1024*1024)throw new Error('备份文件不能超过 8 MB');const backup=JSON.parse(await file.text());if(backup.musicSources!==undefined)validateMusicSources(backup.musicSources);validateNavigation(backup.navigation);validatePageContent(backup.content,backup.navigation.categories);if(!backup.settings||typeof backup.settings.tagline!=='string'||typeof backup.settings.announcement!=='string')throw new Error('请使用本站导出的完整备份文件');const groups=flattenCategories(backup.navigation.categories),total=groups.reduce((sum,c)=>sum+c.sites.length,0);
    showConfirm('恢复备份',`文件：${file.name}\n包含 ${total} 个网站、${groups.length} 个分类、${backup.content.featured.items.length} 个精选、${backup.content.friends.length} 个友链；${backup.musicSources!==undefined?backup.musicSources.length+' 个黑胶音源':'旧版备份不含音源，将保留当前音源'}。\n\n确认后将替换当前全部目录与设置。`,async()=>save(backup,'备份已恢复，前台目录已更新'));
