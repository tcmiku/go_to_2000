...
let data, section='sites', query='', categoryFilter='all', statusFilter='all', page=1, busy=false, selected=new Set(), undoData=null, editorHandler=null, confirmHandler=null, setupRequired=false;
const pageSize=20;
const sections={sites:['▧','网站管理','DIRECTORY','添加、整理和维护导航页中的每一个网站。','添加网站'],categories:['▰','分类管理','CATEGORIES','整理两级目录，调整分类顺序。','添加分类'],hot:['★','热门推荐','RECOMMENDATIONS','挑选值得发现的网站，按顺序展示在前台。','添加推荐'],featured:['✦','精选网站','FEATURED SITES','维护前台精选栏中的网站、简介和封面。','添加精选'],friends:['↗','友情链接','WEB RING','让有趣的小站彼此相连。','添加友链'],settings:['⚙','站点设置','PREFERENCES','修改公告、展示文案和管理员密码。',null],backup:['▣','备份与恢复','BACKUP & RESTORE','带走一份完整目录，随时从备份恢复。',null]};
...
function drawStats(){const all=records();$('#stats-row').innerHTML=[['网站总数',all.length,'DIRECTORY ENTRIES','▧'],['目录分类',flat().length,`${data.navigation.categories.length} TOP LEVEL`,'▰'],['前台显示',all.filter(s=>!s.hidden).length,'PUBLIC SITES','◉'],['精选与友链',data.content.featured.items.length+data.content.friends.length,'HAND-PICKED LINKS','✦']].map(([name,count,note,icon])=>`<div class="stat"><span>${name}</span><strong>${count}</strong><small>${note}</small><i>${icon}</i></div>`).join('');}
function drawNav(){$('#admin-nav').innerHTML=Object.entries(sections).map(([id,[icon,name]])=>`${id==='settings'?'<div class="nav-separator"></div>':''}<button data-section="${id}" class="${section===id?'active':''}" aria-current="${section===id?'page':'false'}"><span>${icon}</span>${name}${['sites','categories'].includes(id)?`<span class="nav-count">${id==='sites'?records().length:flat().length}</span>`:''}</button>`).join('');}
function draw(){
  ...
  if(section==='sites')drawSites();else if(section==='categories')drawCategories();else if(section==='hot')drawHot();else if(section==='featured'||section==='friends')drawLinks();else if(section==='settings')drawSettings();else drawBackup();
}
function siteRows(){
...
function drawSettings(){$('#workspace-body').innerHTML=`<form id="settings-form" class="settings-form"><h3>✎ 前台内容</h3><label>首页标语<input name="tagline" maxlength="100" value="${e(data.settings.tagline)}" required></label><label>站内公告<textarea name="announcement" maxlength="500" required>${e(data.settings.announcement)}</textarea></label><div class="form-grid"><label>热门推荐栏目名称<input name="hotTitle" maxlength="80" value="${e(data.content.hot.title)}" required></label><label>精选栏目名称<input name="featuredTitle" maxlength="80" value="${e(data.content.featured.title)}" required></label></div><p class="form-error" id="settings-error" role="alert"></p><button class="primary" type="submit">▣ 保存设置</button><span class="save-note">保存后立即生效</span></form><form id="password-form" class="password-section"><h3>▣ 修改管理员密码</h3><div class="form-grid"><label>当前密码<input name="current" type="password" required maxlength="200" autocomplete="current-password"></label><label>新密码（至少 10 位）<input name="password" type="password" required minlength="10" maxlength="200" autocomplete="new-password"></label></div><p class="form-error" id="password-error" role="alert"></p><button type="submit">更新密码</button></form>`;}
...
async function loadData(){data=await api('/api/admin/data');selected.clear();draw();}
...
document.addEventListener('submit',async event=>{
  if(event.target.id==='settings-form'){event.preventDefault();if(busy)return;const form=new FormData(event.target);try{await mutate(d=>{d.settings={tagline:String(form.get('tagline')).trim(),announcement:String(form.get('announcement')).trim()};d.content.hot.title=String(form.get('hotTitle')).trim();d.content.featured.title=String(form.get('featuredTitle')).trim();},'站点设置已保存');}catch(error){$('#settings-error').textContent=error.message;}}
